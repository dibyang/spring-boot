...
class SpringProfileIfNestedWithinSecondPhaseElementSanityChecker extends ContextAwareBase implements SanityChecker {
	...
	private static final List<Class<? extends Model>> SECOND_PHASE_TYPES = Arrays.asList(AppenderModel.class,
	...
	public void check(Model model) {
		...
		if (!nestedPairs.isEmpty()) {
			...
			nestedPairs.forEach((nested) -> {
				...
				addWarn(String.format("Element <%s> at line %s contains a nested <%s> element at line %s",
